...
class pure_pursuit:
    def __init__(self):
        ...
        self.ctrl_cmd_pub = rospy.Publisher('/ctr_cmd', CtrlCmd, queue_size=1)
            ...
            for i in range(len(global_ped_info)):
                for path in ref_path.poses:
                    if global_ped_info[i][0] == 0:  # type=0 [pedestrian]
                        ...
                        dis = sqrt(pow(global_ped_info[i][1] - path.pose.position.x, 2) + pow(
                            global_ped_info[i][2] - path.pose.position.y, 2))
                        if dis < 2.35:
            ...
            for i in range(len(global_npc_info)):
                for path in ref_path.poses:
                    if global_npc_info[i][0] == 1:  # type=1 [npc_vehicle]
                        dis = sqrt(pow(global_npc_info[i][1] - path.pose.postion.x, 2) + pow(
                            global_npc_info[i][2] - path.pose.position.y, 2))
                        if dis < 2.35:
            ...
            for i in range(len(global_obs_info)):
                for path in ref_path.poses:
                    if global_obs_info[i][0] == 2:  # type=1 [obstacle]
                        dis = sqrt(pow(global_obs_info[i][1] - path.pose.postion.x, 2) + pow(
                            global_obs_info[i][2] - path.pose.position.y, 2))
                        if dis < 2.35:
