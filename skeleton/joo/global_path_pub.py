...
class global_path_pub :
    def __init__(self, pkg_name = 'ssafy_2', path_name = 'kcity'):
        ...
        # 넣어준 반복 문을 이용하여 작성한다.
        for line in lines :
            tmp = line.split()  # 읽어온 데이터를 쪼개기
            read_pose = PoseStamped()  # 차량의 자세를 read_pose 변수에 기록
            read_pose.pose.position.x = float(tmp[0])  # 정수형으로 변환해줘야 작동한다.
