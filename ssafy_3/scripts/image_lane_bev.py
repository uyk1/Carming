...
def warp_image(img, source_prop):
    ...
    ## source_points : 원본 이미지에서 변환하고자하는 4개의 입력 점에 대한 배열
    ## destination_points : 출력 이미지에서의 대응하는 4개의 점에 대한 배열
    ## 입력된 배열들을 사용하여 3x3 크기의 변환 행렬 M을 생성
    perspective_transform = cv2.getPerspectiveTransform(source_points, destination_points)

    # TODO: (4) 이후 원근 맵 행렬에 대한 기하학적 변환을 진행합니다.

    ## perspective_transform(변환 행렬)을 사용하여 이미지 변환
...
class IMGParser:
    def __init__(self):
        ...
        '''
        ## 위에서 바라본 것처럼 이미지를 변환하기 위해 좌표 조정이 필요
        ## 왼쪽 아래, 왼쪽 위, 오른쪽 위, 오른쪽 아래 순서로 좌표 비율 조정
        ## 아래의 행렬을 [x,y]*4 행렬에 곱할 것
        self.source_prop = np.float32([
            ...
            # [1 - 0.01, 0.65]
            [0.01, 0.7],
            [0.4, 0.53],  ## y를 늘리면 고개를 든 상태에서 보는 것처럼 보임
            [0.6, 0.53],
            [1 - 0.01, 0.7]
        ])
