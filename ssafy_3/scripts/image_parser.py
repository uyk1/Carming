...
class IMGParser:
    ...
    def callback(self, msg):
        try:
            ...
            ## count 읽어올 바이트 수를 지정하는 파라미터 (-1로 지정하면 모든 데이터를 읽어올 수 있다)
            ...
            ## cv2.imdecode() : 바이트 문자열을 이미지로 디코딩하는 함수
            ## 디코딩된 이미지는 넘파이 배열로 반환됨
            ## cv2.IMREAD_COLOR 옵션 : 이미지를 BGR 색상 형식(3차원 배열)으로 반환 가능
            ## BGR로 반환 시, 다른 색상 형식으로 변환 또는 이미지 처리 및 분석을 위해 사용 가능
        ...
        cv2.imshow('image', img_bgr)  ## 이미지 출력 cv2.imshow('열릴 창의 이름', 출력할 이미지)
        cv2.waitKey(1)  ## 이미지 업데이트 시간을 지연시키는 역할. 1ms대기
